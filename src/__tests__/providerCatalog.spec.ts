...
describe("GET /:provider", () => {
  ...
  describe("given the cloudstac/ALL provider/catalog", () => {
    beforeEach(() => {
      sandbox.stub(Provider, "getCloudProviders").resolves([
        null,
        [
    ...
    it("should call the graphql API with no provider search clause", async () => {
      ...
      // If this was any provider other than 'ALL', this method would be
      // called with { provider: 'TEST', cursor: undefined, limit: NaN }
      expect(getCollectionsSpy).to.have.been.calledWith({ cursor: undefined, limit: NaN, cloudHosted: true });
    });
    it("should return rel=child links whose href contains a provider rather than 'ALL'", async () => {
      sandbox
        .stub(Provider, "getProviders")
    ...
    it("should not display any non-cloudhosted providers", async () => {
      ...
      sandbox.stub(Collections, "getCollectionIds")
      .withArgs({ cloudHosted: true }, sinon.match.any).resolves({
        count: 1,
        cursor: "foundCursor",
      ...
      .withArgs({ cloudHosted: false }, sinon.match.any).resolves({
        count: mockCollections.length,
        cursor: "foundCursor",
      ...
      expect(children[0].provider).to.equal("CLOUD_PROV");
      // TODO: this test is broken! it needs to get the provider title and not the collection title.
      // also would need to stub getCollectionIds differently for if mergedQuery includes cloudHosted: true
    })
  });
});
